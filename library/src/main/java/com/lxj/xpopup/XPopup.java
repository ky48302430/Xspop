...
import com.lxj.xpopup.impl.TipPopupView;
...
public class XPopup {
    ...
    public XPopup asTip(int type,String title, String content) {
        return   asTip(type,title,content,null,null);
    }
     ...
     * @return
     */
    public XPopup asTip(int type,String title, String content, OnConfirmListener confirmListener) {
      return   asTip(type,title,content,confirmListener,null);
    }

    public XPopup asTip(int type,String title, String content, OnConfirmListener confirmListener,OnCancelListener cancelListener) {
        position(PopupType.Center);

        TipPopupView popupView = new TipPopupView(contextRef.get());
        popupView.setTitleContent(title, content);
        popupView.setListener(confirmListener, cancelListener);
        popupView.setAlertType(type);
    ...
    public XPopup asLoading(String title) {
        position(PopupType.Center);
        this.tempView = new LoadingPopupView(contextRef.get()).setTitle(title);
        return this;
    }



    public XPopup asLoading(String title,int type) {
        position(PopupType.Center);
        LoadingPopupView  loadingPopupView = new LoadingPopupView(contextRef.get()).setTitle(title);
        loadingPopupView.setType(type);
        this.tempView = loadingPopupView;
        return this;
    }
